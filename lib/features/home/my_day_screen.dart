...
// 🎯 FIX: ADD THIS PUBLIC TYPEDEF TO EXPOSE THE STATE TYPE
  ...
  const MyDayScreen({
    ...
    // ✅ TO THIS (it's no longer required):
    this.onBellClick,
...
// --- STATE for Tracking ---
  ...
  Row _buildTrailingActions(BuildContext context, VoidCallback refreshCallback) {
    return Row(
      ...
      children: [
        // Wrap the ValueListenableBuilder's result in a GestureDetector
        // 3. ✅ ADD THIS `if` CHECK:
        // Only show the bell icon if the callback was provided
        if (widget.onBellClick != null)
          ValueListenableBuilder<int>(
            valueListenable: NotificationService.instance.badgeNotifier,
            builder: (context, count, child) {
              return GestureDetector(
                onTap: widget.onBellClick, // This will only be non-null if the if-check passes
                child: BadgeIcon(

...
class PunchEventDetail {
  final String type; // e.g., "PUNCH 1 IN", "BREAK"
    ...
    with AutomaticKeepAliveClientMixin<TimesheetScreen>
{
  ...
  Future<void> _loadAllTimesheetData() async {
    ...
    try {
      ...
      // 1. Fetch Summary Data (New API)
      final summaryFuture = DashboardService.instance.getAttendanceTimesheetData();

      // 2. Fetch Timesheet Grid Data (Raw data for detail panel)
      final rowsFuture = TimesheetService.instance.getRangeRaw(
        start: _weekStartSun,
        end: _weekEndSat,
      );

      // Wait for both futures concurrently
      final results = await Future.wait([summaryFuture, rowsFuture]);
      final summary = results[0] as AttendanceTimesheetData;
      ...
      if (!mounted) return;
      setState(() {
        // --- Update Summary Stats from new API (for static top cards) ---
        ...
        // This is the static value for the top card (today's yesterday)
  ...
  // 🎯 _minsOfRow (The duration parser)
  int _minsOfRow(Map<String, dynamic> row) {
    ...
    String _safeTime(dynamic v) { /* ... */ return '—'; }
    int _toMin(String hhmm) { /* ... */ return 0; }
  ...
  // 🎯 REFACTOR: Change return type to dynamic list of PunchEventDetail
  List<PunchEventDetail> _getDynamicPunchesForSelected() {
    ...
    // We determine the punch set number based on IN punches
    for (final e in evts) {
      final dt = _asDateTime(e['eventTime'] ?? e['time'] ?? e['timestamp'] ?? e['at']);
      if (dt == null || !_isSameDay(dt, _selectedDate)) continue;
  ...
  // 🎯 NEW: Helper function to map raw status to display data
  _PillData _getPillData(String rawStatus) {
    ...
    switch (status) {
      ...
      case 'PENDING':
      case 'IN PROGRESS':
        return const _PillData('In Progress', Color(0xFFFFC107), Colors.white);
      case 'VIOLATION':
      case 'EXCEPTION':
      case 'REJECTED':
        return const _PillData('Violation', Color(0xFFDC3545), Colors.white);
      default:
      // 🎯 FIX: Use the actual status string if it exists, otherwise fallback to generic
              ...
              child: Padding(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    // 🎯 CRITICAL FIX: Loop dynamically over punches with simplified labels
                    if (dynamicPunches.isEmpty)
                      const KeyVal('Punches', 'No recorded punches for this day.')
                    else
                    // Use KeyVal for each punch event
                      ...dynamicPunches.map((punch) {
                        return KeyVal(
                          punch.type, // e.g., "Punch 1 IN", "BREAK"
                          punch.time, // e.g., "09:00"
                        );
                      }).toList(),
                    ...
                    Row(
                      children: [
                        // 🎯 USE CALCULATED DURATION FOR SELECTED DAY
                        Expanded(child: DetailTile(label: 'Total Hours', value: selectedDurLabel)),
                        // 🎯 USE CALCULATED EXCEPTION STATUS FOR SELECTED DAY
